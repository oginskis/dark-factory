...
- Provide the file paths from the table above when the workflow references logical resources (e.g., "the company report", "the catalog assessment", "the product taxonomy categories file", "the SKU schema", "the platform knowledgebase", "the generator input file", "the language seed file").
...
- **SKU schema enforcement:** Subcategories without SKU schemas are skipped — never auto-generated. Before concluding a schema is missing, **list `docs/product-taxonomy/sku-schemas/`** — never guess the filename. The `no_sku_schema` decision is autonomous (skip and report). If ALL subcategories lack schemas, the `all_subcategories_lack_schemas` decision escalates to the user.
...
- `invalid_taxonomy_id` — 1) Re-run `/product-classifier {slug}` to reclassify the company with valid taxonomy IDs, then retry, 2) Stop
- `all_subcategories_lack_schemas` — 1) Run `/product-taxonomy {subcategory name}` for the primary subcategory to generate the schema, then retry, 2) Stop
